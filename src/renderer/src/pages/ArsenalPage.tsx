...
import { defaultEffectChain, EFFECT_BLOCKS, type ArsenalPreset, type EffectBlock, type EffectChainSnapshot, type MonitorMode } from '@shared/arsenal.js'
import './arsenal.css'

const labels: Record<EffectBlock, string> = { amp: 'AMP + CAB', eq: 'EQ', delay: 'DELAY', reverb: 'REVERB' }
const colors: Record<EffectBlock, string> = { amp: '#252321', eq: '#d6d3cb', delay: '#efe2cf', reverb: '#286a9a' }
...
export function ArsenalPage({ onToast }: { onToast(message: string): void }): React.JSX.Element {
  ...
  useEffect(() => { void window.bandbuddy.arsenal.list().then((state) => { setPresets(state.presets); const first = state.presets[0]; if (first) { setSelectedId(first.id); setChain(first.chain) } }) }, [])
  ...
  const selectPreset = (preset: ArsenalPreset): void => { setSelectedId(preset.id); setChain(structuredClone(preset.chain)); setDirty(false) }
  ...
  const enabled = (block: EffectBlock): boolean => block === 'amp' ? chain.amp.enabled : block === 'eq' ? chain.eq.enabled : block === 'delay' ? chain.delay.enabled : chain.reverb.enabled
  return <main className="arsenal-page">
    <aside className="arsenal-sidebar">
      ...
      <div className="arsenal-preset-list">{visible.map((preset) => <button key={preset.id} className={`arsenal-preset ${preset.id === selectedId ? 'selected' : ''}`} onClick={() => selectPreset(preset)}><span className="preset-thumb" style={{ background: preset.id === selectedId ? 'linear-gradient(145deg,#9c7a55,#382d23)' : 'linear-gradient(145deg,#676159,#25221f)' }} /><span><b>{preset.name}</b><small>{preset.chain.amp.enabled ? 'NAM' : '干声'} · {preset.chain.delay.enabled ? '延迟' : '直达'} · {preset.chain.reverb.enabled ? '空间' : '无混响'}</small></span><i>⋮</i></button>)}</div>
    ...
    <section className="arsenal-workspace">
      <header className="arsenal-toolbar"><div><span className="arsenal-kicker">SIGNAL WORKSHOP</span><h2>{presets.find((p) => p.id === selectedId)?.name ?? '新预设'}{dirty && <sup>未保存</sup>}</h2></div><div className="arsenal-toolbar-actions"><button className="outline-button compact" onClick={() => void importAsset('nam')}><Upload size={14} />导入 NAM</button><button className="outline-button compact" onClick={() => void importAsset('ir')}><Upload size={14} />导入 IR</button></div></header>
